...
function Sprite (_config) {
	...
	var defaults = {
		...
		selector: function (filename, tokens) {
			// Added state variable & added placeholders fors Sass
			var state = "";
			if (tokens.state) {
			...
			return "." + parts.join("-") + state + (tokens.state ? ', %' + parts.join("-") + state.replace(':', '-') : '');
		},
		stateToken: null,
	...
	*/
	...
	if (config.spritePath.match(/\.svg$/)) {
		spriteName = path.basename(config.spritePath);
		svgPath = config.spritePath;
	}
	else {
		spriteName = util.joinName(config.prefix, config.name, "sprite") + ".svg";
	...
	this.config = config;
	this.baseIconClass = "." + config.baseIconClass;
...
Sprite.prototype.addSize = function (label) {
	...
	var pngName = this.spriteName.replace(/\.svg$/, ((label) ? "-" + label : "") + ".png");
